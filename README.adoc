...
* https://github.com/lyrasis/kiba-extend-project/blob/main/lib/omca.rb[configuring your project]
* https://github.com/lyrasis/kiba-extend-project/blob/main/lib/omca/registry_data.rb[registering your project's jobs/files]
* project-specific kiba transforms
** https://github.com/lyrasis/kiba-extend-project/blob/main/lib/omca/transforms/locations/loc_name_reverser.rb[defining]
** https://github.com/lyrasis/kiba-extend-project/blob/main/spec/omca/transforms/locations/loc_name_reverser_spec.rb[testing]
** https://github.com/lyrasis/kiba-extend-project/blob/4e4458ee7a9ed7e56e0a5e88b82a3b6bcf1fc89d/lib/omca/source_system/locations.rb#L40[using in jobs]
...
Everything in the https://github.com/lyrasis/kiba-extend-project/tree/main/lib/omca/jobs[/lib/omca/jobs] is an example of how job transformation logic is typically set up.
The https://github.com/lyrasis/kiba-extend-project/blob/main/lib/omca/everything_exploded.rb[everything_exploded.rb] module is a heavily commented, deconstructed job demonstrating how each piece of a job works.
...
To understand the relationship between `lib/omca/registry_data.rb` and setting up jobs, I recommend you start by looking at that file and `lib/omca/everything_exploded.rb`, which has more comments explaining how this works. Then look at the files in `lib/omca/locations` for the current recommended approach.
...
* global find/replace `Omca` and `omca` with your project name
...
Configure your project settings in your `lib/omca.rb` equivalent.
...
With the exception of `registry_data.rb`, the structure of directories and files in `lib/omca` is fairly arbitrary, in the sense of how you name and structure modules and submodules, and what methods you define in different modules. For example, you _could_ define all your jobs in one big module file if you wanted.

However, the author has thus far found it useful to set up the structure has shown in the `lib/omca/jobs/locations` directory.
